...
// 递归只负责打印 u 和 v 之间的“中间人”
void findPath(int u, int v, int path[][maxsize]) {
    int m = path[u][v];
    if (m == -1) return; // u 到 v 之间没有中转点，直接返回

    findPath(u, m, path);  // 递归找左边的中转点
    printf("%d ", m);      // 打印当前发现的最优中转点
    findPath(m, v, path);  // 递归找右边的中转点
}

// 调用入口
void printPath(int u, int v, int path[][maxsize]) {
    printf("%d ", u);      // 打印起点
    findPath(u, v, path);  // 打印所有中间点
    printf("%d ", v);      // 打印终点
}
```
