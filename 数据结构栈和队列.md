...
void fun(char A[]) {
    ...
    for (int i = 0; A[i] != '\0'; i++) {
        if (A[i] == 'S')
            ...
            s.data[++s.top] = A[i];  // 将 H 入栈
    }

    while (s.top != -1)
        A[k++] = s.data[s.top--];  // 将栈中 H 弹出，放在后面
}
```
...
**判断子串 s2 是否匹配母串 s1，若匹配，输出匹配到的第一个字符所在索引。否则输出-1。**
...
**有两个链表 A 和 B，判断 B 是否是 A 的连续子序列**

```c
int find(LinkList A, LinkList B){
    LinkList pos = A->next;
