...
题目：已知 2000 年 1 月 1 日是星期六，现给定三个整数 year, month, day 分别代表某年某月某日，请你计算并输出数字 1-7 表示给定的日期星期几。题目保证输入日期大于 2000 年 1 月 1 日。

**输入**

- 三个数分别代表 year, month, day，空格分开
...
int func(int year, int month, int day){
    // 记录每个月有多少天，索引从 1 开始用比较方便对应
    int monthDayMap[13] = {0, 31, 28, 31, 30, 31, 30, 31, 
                           31, 30, 31, 30, 31};
    int totalDays = 0; // 记录总共过去了多少天

    for(int i = 2000; i < year; i++)  // 从 2000 年加到当前年，注意闰年 366 天
        totalDays += isLeapYear(i) ? 366 : 365;

    for(int i = 1; i < month; i++)    // 从一月加到当前月，但不包含当月
        totalDays += monthDayMap[i];  // 这里索引从 1 开始所以没问题

    totalDays += day - 1; // 添加当前天数，减 1 是因为 2000.1.1 已经算一天了

    if(isLeapYear(year) && month > 2) // 处理当年是闰年且输入日期大于 2 月的情况
        totalDays++;

    // 计算星期，7 天一循环，从 1 月 1 日开始是星期六索引是 5，取余 7 的范围是 0-6
    // 但是我们要 1-7 所以加 1 偏移
    int weekNumber = (5 + totalDays) % 7 + 1;
    return weekNumber;
}
...
题目：假设你向银行贷款，银行的利息计算规则为：当利率为 p% 时，如果上个月欠 x 元，则下个月应还款额为 x × (1 + p%) 元。现在已知你最开始欠银行 m 元，银行利率为 p%，从下个月开始还款，并打算每个月固定还 n 元，请编程实现，计算你需要还款多少次还清。
...
// 如果只是简单的用 double 还款，则举例子会发现如果还 10 元，每个月还 0.01 元，0 利息
// 情况下需要还 1001 次，这就是浮点数带来的误差的问题
// 正确做法是把钱都转为分单位并用整数存储来进行计算，分是最小单位，低于分的都
// 直接忽略，这样就可以保证精度。double 类型不建议在涉及钱的运算中会有误差
// 所以这里我们用 int oneYuan = 100; 而不是 double oneYuan = 0.01
// 还款顺序是先加上利息再扣除固定还款额

int predict(double m, double n, double p){
    int month = 0;
    // 模拟还款过程，以分为单位并用整数存储进行计算
    int cent = (int)(m * 100);
    int repay = (int)(n * 100);
    while(cent > 0) {
        // 由于是下个月还款，所以先涨利息再还款
        // 这里强制转为整数不用担心精度问题，因为分是最低单位，低于分的钱可以直接忽略
        cent = (int)(cent * (1 + p / 100));
        cent -= repay;
        month++;
    }
    return month;
...
题目：请用二分法求方程 *y*=*x*2−4(*x*>0) 的近似根，初始化区间的左右端点分别为 0 和 5, 当区间长度小于 0.01 时停止迭代，并输出方程的近似根，保留 5 位小数

输入：无

输出: 2.00195 (二分法迭代近似的结果)

```c
#include <stdio.h>
//定义要寻找根的函数
double foo(double x){
    return x * x - 4; //寻找x^2 - 4 = 0 的根
}
//二分法求根, 和二分查找类似, 都是不断迭代地把区间折半的思想
//threshold代表left和right的距离阈值
//当他们的距离小于threshold说明二者已经非常接近了, 那么root也就非常接近根
//比如当a=1.999, b=2.001时, 二者之差为0.002<threshold=0.01, 根就夹杂在这中间所以没必要迭代了
//还有一些关于迭代求根的方法, 梯度下降法求根, 牛顿法求根, 有些学校也考, 原理也很简单, 大家可以自行学习试一下, 也可以给助教查看代码
double binarySection(double a, double b, double c){
    //初始的选点一定要保证f(a)*f(b)异号, 零点定理, 这题是x=0和x=5
    double root; //和二分查找的mid变量类似
    while ((b - a) >= c){
        //计算中点并判断是否为根
        root = (a + b) / 2;
        //运气很好刚好找到根
        if (foo(root) == 0)
            break;
        //根在左半边, 把b左移到root
        else if (foo(root) * foo(a) < 0)
            b = root;
        //根在右半边, 把a右移到root
        else
            a = root;
    }
    printf("%.5lf", root); //输出近似根
    return root;
}
int main() {
    double a = 0, b = 5; //定义区间
    double threshold = 0.01; //定义阈值
    double root = binarySection(a, b, threshold);
    printf("%.5f\n", root);
    return 0;
}
```
...
题目：编程一个递归函数，求一个整型数组的元素中的最大值

输入：第一行一个整数 n，第二行输入 n 个整型数据，用空格分开
...
```c
#include <stdio.h>
//一个长度为n的数组的最大值可以分为：第n 个元素的值和长度为n-1 的子数组中的更大的那一个，非递归的写法我也不写了
int getMaxVal(int arr[], int n){
    if(n == 1)
        return arr[0];
    else
        return arr[n - 1] > getMaxVal(arr, n - 1) ? arr[n - 1] : getMaxVal(arr, n - 1);
}
int main(){
    int length;
    scanf("%d", &length);
    ...
    for(int i = 0; i < length; i++)
        scanf("%d", &arr[i]);
    int maxval = getMaxVal(arr, length);
    printf("%d", maxval);
    return 0;
}
```
